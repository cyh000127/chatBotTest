from datetime import datetime

BUTTON_TODAY_DATE = "កាលបរិច្ឆេទថ្ងៃនេះ"
BUTTON_TODAY_WEATHER = "អាកាសធាតុថ្ងៃនេះ"
BUTTON_FERTILIZER = "បញ្ចូលជី"
BUTTON_YIELD = "បញ្ចូលទិន្នផល"
BUTTON_MYFIELDS = "ពិនិត្យផ្ទាល់ខ្លួន"
BUTTON_EVIDENCE = "ដាក់ស្នើភស្តុតាង"
BUTTON_FIELD_REGISTER = "ចុះបញ្ជីស្រែ"
BUTTON_FIELD_REFRESH = "ផ្ទុកឡើងវិញ"
BUTTON_FIELD_LOOKUP_LOCATION = "រកតាមទីតាំង"
BUTTON_FIELD_LOOKUP_CODE = "បញ្ចូលលេខកូដស្រែ"
BUTTON_INPUT_RESOLVE = "បកស្រាយតម្លៃ"
BUTTON_INPUT_RESOLVE_TARGET_FIELD_CODE = "លេខកូដស្រែ"
BUTTON_INPUT_RESOLVE_TARGET_FIELD_NAME = "ឈ្មោះស្រែ"
BUTTON_INPUT_RESOLVE_METHOD_TYPED_TEXT = "វាយអក្សរ"
BUTTON_INPUT_RESOLVE_CONFIRM_CANDIDATE = "យកជម្រើសនេះ"
BUTTON_INPUT_RESOLVE_RETRY = "សាកម្តងទៀត"
BUTTON_INPUT_RESOLVE_RETRY_LATER = "ពេលក្រោយ"
BUTTON_INPUT_RESOLVE_MANUAL_REVIEW = "ពិនិត្យដោយមនុស្ស"
BUTTON_SUPPORT = "ជំនួយ"
BUTTON_HELP = "ជំនួយ"
BUTTON_BACK = "ថយក្រោយ"
BUTTON_CANCEL = "បោះបង់"
BUTTON_RESTART = "ចាប់ផ្តើមឡើងវិញ"
BUTTON_CONFIRM = "បញ្ជាក់"
BUTTON_EDIT = "កែប្រែ"
BUTTON_EDIT_START = "ចាប់ផ្តើមកែ"
BUTTON_KEEP_CURRENT = "ទុកដដែល"
BUTTON_APPLY_SUGGESTED_VALUE = "ប្រើតម្លៃនេះ"
BUTTON_ENTER_VALUE_DIRECTLY = "បញ្ចូលដោយដៃ"
BUTTON_YES = "បាទ/ចាស"
BUTTON_NO = "ទេ"
BUTTON_EDIT_NAME = "ឈ្មោះ"
BUTTON_EDIT_PHONE = "លេខទូរស័ព្ទ"
BUTTON_EDIT_RESIDENCE = "ទីលំនៅ"
BUTTON_EDIT_CITY = "រាជធានី/ខេត្ត"
BUTTON_EDIT_DISTRICT = "ខណ្ឌ/ស្រុក/ក្រុង"
BUTTON_EDIT_BIRTH_DATE = "ថ្ងៃខែឆ្នាំកំណើត"
BUTTON_FERTILIZER_EDIT_USED = "ការប្រើជី"
BUTTON_FERTILIZER_EDIT_KIND = "ប្រភេទជី"
BUTTON_FERTILIZER_EDIT_PRODUCT = "ឈ្មោះផលិតផល"
BUTTON_FERTILIZER_EDIT_AMOUNT = "បរិមាណ"
BUTTON_FERTILIZER_EDIT_DATE = "ថ្ងៃប្រើ"
BUTTON_YIELD_EDIT_READY = "ស្ថានភាពរួចរាល់"
BUTTON_YIELD_EDIT_FIELD = "ស្រែ"
BUTTON_YIELD_EDIT_AMOUNT = "បរិមាណទិន្នផល"
BUTTON_YIELD_EDIT_DATE = "ថ្ងៃប្រមូលផល"
BUTTON_PREV_YEARS = "12 ឆ្នាំមុន"
BUTTON_NEXT_YEARS = "12 ឆ្នាំបន្ទាប់"
BUTTON_FERTILIZER_KIND_COMPOUND = "ជីចម្រុះ"
BUTTON_FERTILIZER_KIND_UREA = "យូរ៉េ"
BUTTON_FERTILIZER_KIND_COMPOST = "ជីកំប៉ុស"
BUTTON_FERTILIZER_KIND_LIQUID = "ជីរាវ"

LANGUAGE_NAME = "ខ្មែរ"
LANGUAGE_MENU_MESSAGE = "សូមជ្រើសភាសា។"
LANGUAGE_CHANGED_MESSAGE = "បានប្តូរទៅភាសាខ្មែរ។"
AUTH_START_PROMPT = "សូមបញ្ចូលលេខសម្គាល់ចូល។\nឧទាហរណ៍៖ /start sample-user ឬ sample-user"
AUTH_REQUIRED_MESSAGE = "ត្រូវចូលប្រើជាមុនសិន។\nសូមវាយ /start ហើយបញ្ចូលលេខសម្គាល់របស់អ្នក។"
AUTH_INVALID_MESSAGE = "លេខសម្គាល់មិនត្រឹមត្រូវ។ សូមបញ្ចូលម្ដងទៀត។"
AUTH_RETRY_LIMIT_MESSAGE = "បញ្ចូលខុស 2 ដងហើយ។\nសូមវាយ /start ម្តងទៀត។"
AUTH_WELCOME_MESSAGE = "សូមស្វាគមន៍ {user_name}!"
AUTH_ALREADY_LOGGED_IN_MESSAGE = "សូមស្វាគមន៍មកវិញ {user_name}!"
START_REQUIRED_MESSAGE = "សូមចាប់ផ្តើមដោយវាយ /start ជាមុនសិន។"
ONBOARDING_INVITE_REQUIRED_MESSAGE = "ត្រូវការកូដអញ្ជើញ។\nបើអ្នកមានកូដ សូមចាប់ផ្តើមដោយ /start <invite_code>។"
ONBOARDING_INVALID_INVITE_MESSAGE = "មិនអាចផ្ទៀងផ្ទាត់កូដអញ្ជើញបានទេ។\nសូមពិនិត្យកូដ ហើយវាយ /start <invite_code> ម្តងទៀត។"
ONBOARDING_IDENTITY_REQUIRED_MESSAGE = "មិនអាចអានព័ត៌មានអ្នកប្រើ Telegram បានទេ។\nសូមព្យាយាមម្តងទៀតបន្តិចក្រោយ។"
ONBOARDING_STARTED_MESSAGE = "បានផ្ទៀងផ្ទាត់កូដអញ្ជើញ។\nសូមជ្រើសភាសា ដើម្បីបន្តការចុះឈ្មោះ។"
ONBOARDING_ACCESS_REQUIRED_MESSAGE = "ត្រូវបញ្ចប់ការចុះឈ្មោះ និងទទួលការអនុម័តពីអ្នកគ្រប់គ្រងជាមុន។\nបើអ្នកមានកូដអញ្ជើញ សូមចាប់ផ្តើមដោយ /start <invite_code>។"
ONBOARDING_PENDING_APPROVAL_MESSAGE = "ការចុះឈ្មោះមិនទាន់បញ្ចប់ ឬកំពុងរង់ចាំការអនុម័ត។\nអ្នកអាចប្រើសេវាបានបន្ទាប់ពីអនុម័ត។"
ONBOARDING_NAME_PROMPT = "សូមបញ្ចូលឈ្មោះ។"
ONBOARDING_NAME_FALLBACK = "សូមបញ្ចូលឈ្មោះម្ដងទៀត។"
ONBOARDING_NAME_REPAIR_PROMPT = "សូមបញ្ចូលឈ្មោះថ្មី។"
ONBOARDING_PHONE_PROMPT = "សូមបញ្ចូលលេខទូរស័ព្ទជាមួយលេខកូដប្រទេស។\nឧទាហរណ៍៖ +855 12 345 678, +880 17 1234 5678"
ONBOARDING_PHONE_FALLBACK = "សូមបញ្ចូលលេខទូរស័ព្ទម្ដងទៀត។"
ONBOARDING_PHONE_REPAIR_PROMPT = "សូមបញ្ចូលលេខទូរស័ព្ទថ្មី។"
ONBOARDING_CONFIRM_MESSAGE = (
    "សូមពិនិត្យព័ត៌មានចុះឈ្មោះ។\n"
    "- ឈ្មោះ៖ {name}\n"
    "- លេខទូរស័ព្ទ៖ {phone}\n"
    "- ភាសា៖ {language}\n\n"
    "បើត្រឹមត្រូវ សូមចុច [បញ្ជាក់] ឬជ្រើសធាតុដើម្បីកែ។"
)
ONBOARDING_CONFIRM_FALLBACK = "សូមចុច [បញ្ជាក់] ឬជ្រើសធាតុដែលចង់កែ។"
ONBOARDING_PENDING_APPROVAL_SUBMITTED_MESSAGE = "បានដាក់សំណើចុះឈ្មោះសម្រាប់ការអនុម័ត។\nអ្នកអាចប្រើសេវាបានបន្ទាប់ពីអ្នកគ្រប់គ្រងអនុម័ត។"

CITY_BUTTON_TO_KEY = {
    "សេអ៊ូល": "서울",
    "ប៊ូសាន": "부산",
    "ជេជូ": "제주",
}

START_MESSAGE = "បានចាប់ផ្តើម។\nសូមជ្រើសរើសសកម្មភាពខាងក្រោម។"
MAIN_MENU_MESSAGE = "ម៉ឺនុយមេ។\nសូមជ្រើសរើសសកម្មភាពខាងក្រោម។"
WEATHER_MENU_MESSAGE = "សូមជ្រើសទីក្រុងដើម្បីមើលអាកាសធាតុថ្ងៃនេះ។"
PROFILE_ENTRY_MESSAGE = (
    "ចាប់ផ្តើមបញ្ចូលប្រវត្តិ។\n"
    "ដំបូងសូមបញ្ចូលឈ្មោះ។"
)
CANCEL_MESSAGE = "បានចេញពីលំហូរបច្ចុប្បន្ន។\nសូមចាប់ផ្តើមឡើងវិញ ឬវាយ /start ម្តងទៀត។"
BACK_LIMIT_MESSAGE = "មិនមានជំហានមុនទេ។\nសូមជ្រើសម្តងទៀតពីម៉ឺនុយមេ។"
BACK_TO_MAIN_MESSAGE = "បានត្រឡប់ទៅជំហានមុន។\nសូមជ្រើសម្តងទៀត។"
BACK_TO_WEATHER_MESSAGE = "បានត្រឡប់ទៅជំហានជ្រើសអាកាសធាតុ។\nសូមជ្រើសទីក្រុងម្តងទៀត។"
BACK_GENERIC_MESSAGE = "បានត្រឡប់ទៅជំហានមុន។"
WEATHER_ERROR_MESSAGE = "មិនអាចយកព័ត៌មានអាកាសធាតុបាន។\nសូមព្យាយាមម្តងទៀតបន្តិចក្រោយ។"
UNKNOWN_COMMAND_MESSAGE = "ពាក្យបញ្ជានេះមិនទាន់គាំទ្រ។\nសូមជ្រើសធាតុខាងក្រោម ឬប្រើ /help។"
MYFIELDS_ENTRY_MESSAGE = "នេះគឺជាទំព័រពិនិត្យព័ត៌មានរបស់អ្នក។\nពេលនេះមានតែការមើលព័ត៌មាន និងត្រឡប់ក្រោយប៉ុណ្ណោះ។\nសូមជ្រើសសកម្មភាពខាងក្រោម ឬចាប់ផ្តើមម្ដងទៀតដោយ /start។"
MYFIELDS_EMPTY_MESSAGE = "មិនទាន់មានស្រែដែលបានចុះបញ្ជីទេ។\nសូមចាប់ផ្តើមការចុះបញ្ជីស្រែ។"
MYFIELDS_SUMMARY_MESSAGE = "នេះគឺជាស្រែដែលបានចុះបញ្ជីរបស់អ្នក។"
MYFIELDS_SUMMARY_GUIDANCE = "អ្នកអាចចុះបញ្ជីស្រែបន្ថែម ឬផ្ទុកបញ្ជីឡើងវិញបាន។"
MYFIELDS_METHOD_PROMPT = "សូមជ្រើសវិធីចុះបញ្ជីស្រែ។\nអាចប្រើទីតាំង ឬលេខកូដស្រែ។"
MYFIELDS_LOCATION_PROMPT = "សូមផ្ញើទីតាំងបច្ចុប្បន្នតាម Telegram attachment menu។\nបើកម៉ឺនុយភ្ជាប់នៅប្រអប់សារ ហើយជ្រើស Location។"
MYFIELDS_CODE_PROMPT = "សូមបញ្ចូលលេខកូដស្រែ។"
MYFIELDS_CODE_REPAIR_PROMPT = "សូមបញ្ចូលលេខកូដស្រែថ្មី។"
MYFIELDS_LOCATION_TO_CODE_REPAIR_PROMPT = "រកមិនឃើញស្រែត្រូវគ្នាពីទីតាំងនេះទេ។\nសូមបញ្ចូលលេខកូដស្រែ។"
MYFIELDS_NO_REGISTRY_MESSAGE = "មិនទាន់មានបញ្ជីស្រែដែលបានបើកប្រើទេ។\nសូមសាកល្បងម្តងទៀតពេលក្រោយ ឬស្នើជំនួយ។"
MYFIELDS_LOCATION_NO_CANDIDATE_MESSAGE = "មិនអាចរកឃើញស្រែសមរម្យសម្រាប់ទីតាំងនេះទេ។\nសូមបញ្ចូលលេខកូដស្រែ ឬសាកល្បងម្តងទៀត។"
MYFIELDS_CODE_NOT_FOUND_MESSAGE = "មិនអាចរកឃើញលេខកូដស្រែនេះទេ។\nសូមបញ្ចូលម្ដងទៀត ឬប្តូរទៅការរកតាមទីតាំង។"
MYFIELDS_CANDIDATE_PROMPT = "រកឃើញស្រែដែលពាក់ព័ន្ធ។\nសូមជ្រើសស្រែដែលត្រូវ។"
MYFIELDS_CONFIRM_MESSAGE = "សូមពិនិត្យការភ្ជាប់ស្រែ។"
MYFIELDS_CONFIRM_GUIDANCE = "បើត្រឹមត្រូវ សូមចុច [បញ្ជាក់]។"
MYFIELDS_BINDING_CONFIRMED_MESSAGE = "បានចុះបញ្ជីស្រែរួចហើយ។"
MYFIELDS_BINDING_CONFLICT_MESSAGE = "មិនអាចចុះបញ្ជីស្រែនេះភ្លាមៗបានទេ។\nវាត្រូវបានផ្ញើទៅការពិនិត្យប្រតិបត្តិការ។ សូមជ្រើសវិធីផ្សេង។"
MYFIELDS_LOOKUP_METHOD_LOCATION_LABEL = "ទីតាំង"
MYFIELDS_LOOKUP_METHOD_CODE_LABEL = "លេខកូដស្រែ"
EVIDENCE_RUNTIME_UNAVAILABLE_MESSAGE = "មុខងារដាក់ស្នើភស្តុតាងមិនអាចប្រើបានឥឡូវនេះទេ។\nសូមសាកម្តងទៀតនៅពេលក្រោយ ឬស្នើសុំជំនួយ។"
EVIDENCE_ENTRY_MESSAGE = "ចាប់ផ្តើមដាក់ស្នើភស្តុតាង។\nសូមផ្ញើទីតាំងបច្ចុប្បន្នជាមុនសិន។"
EVIDENCE_DOCUMENT_PROMPT = "ទីតាំងត្រូវបានទទួលយកហើយ។\nឥឡូវសូមអាប់ឡូតឯកសារដើមជា document។"
EVIDENCE_LOCATION_FALLBACK = "ជំហាននេះត្រូវការទីតាំងបច្ចុប្បន្នរបស់អ្នក។\nសូមប្រើការភ្ជាប់ទីតាំងរបស់ Telegram។"
EVIDENCE_DOCUMENT_FALLBACK = "ជំហាននេះត្រូវការឯកសារដើមជា document upload។\nសូមប្រើ document attachment ជំនួសរូបថត។"
EVIDENCE_VALIDATING_MESSAGE = "ឯកសារភស្តុតាងត្រូវបានទទួលហើយ។\nសូមរង់ចាំការណែនាំបន្ទាប់ ឬជ្រើសសកម្មភាពដែលពាក់ព័ន្ធ។"
EVIDENCE_ACCEPTED_MESSAGE = "ការដាក់ស្នើភស្តុតាងបានបញ្ចប់ហើយ។\nការត្រួតពិនិត្យមូលដ្ឋានបានឆ្លងកាត់។"
EVIDENCE_RETRY_REQUIRED_MESSAGE = "សូមដាក់ស្នើភស្តុតាងម្ដងទៀត។"
EVIDENCE_MANUAL_REVIEW_MESSAGE = "ការដាក់ស្នើភស្តុតាងត្រូវបានបញ្ជូនទៅការត្រួតពិនិត្យដោយមនុស្ស។"
EVIDENCE_REASON_MISSING_EXIF = "មិនមានព័ត៌មាន EXIF ទេ។"
EVIDENCE_REASON_MISSING_GPS = "មិនមានព័ត៌មាន GPS ទេ។"
EVIDENCE_REASON_MISSING_CAPTURE_TIME = "មិនមានព័ត៌មានពេលថតទេ។"
EVIDENCE_REASON_LOCATION_DISTANCE_TOO_FAR = "ចម្ងាយរវាងទីតាំងបច្ចុប្បន្ន និង GPS រូបថតឆ្ងាយពេក។"
EVIDENCE_REASON_UNSUPPORTED_FILE_TYPE = "អាចផ្ញើបានតែឯកសារ JPEG ដើមប៉ុណ្ណោះ។"
EVIDENCE_REASON_ARTIFACT_READ_FAILED = "មិនអាចអានមេតាទិន្នន័យឯកសារបានទេ។ សូមផ្ញើឯកសារ JPEG ដើមម្តងទៀត។"
INPUT_RESOLVE_ENTRY_MESSAGE = "នេះគឺជាទំព័រជួយបញ្ជាក់តម្លៃ។\nប្រព័ន្ធនឹងទាញយកតម្លៃសំខាន់ៗជាដំបូង។\nការជ្រើសចុងក្រោយ និងការពិនិត្យដោយមនុស្ស ធ្វើនៅជំហានក្រោយ។"
INPUT_RESOLVE_TARGET_FALLBACK = "សូមជ្រើសតម្លៃដែលត្រូវបញ្ជាក់ដោយប៊ូតុង។"
INPUT_RESOLVE_METHOD_FALLBACK = "ជំហាននេះអាចប្រើបានតែការវាយអក្សរ។\nសូមជ្រើស [វាយអក្សរ]។"
INPUT_RESOLVE_RAW_INPUT_FALLBACK = "សូមបញ្ចូលតម្លៃដើមម្ដងទៀត។\nឧទាហរណ៍: FIELD-001, Plot 1"
INPUT_RESOLVE_RAW_INPUT_REPAIR_PROMPT = "រកមិនឃើញបេក្ខភាពត្រូវគ្នាទេ។\nសូមបញ្ចូលតម្លៃថ្មី។"
INPUT_RESOLVE_CANDIDATE_NONE_MESSAGE = "រកមិនឃើញបេក្ខភាពត្រូវគ្នាទេ។\nសូមជ្រើស [បញ្ចូលម្តងទៀត], [ពេលក្រោយ], ឬ [ពិនិត្យដោយដៃ]។"
INPUT_RESOLVE_MANUAL_REVIEW_MESSAGE = "សំណើបកស្រាយតម្លៃត្រូវបានផ្ញើទៅការពិនិត្យដោយមនុស្សហើយ។\nការណែនាំបន្ថែមនឹងបន្តនៅក្នុងបន្ទប់ជជែកនេះ។"
INPUT_RESOLVE_INVALID_RESUME_MESSAGE = "មិនអាចផ្ទៀងផ្ទាត់ព័ត៌មានបន្តបានទេ។\nសូមចាប់ផ្តើមថ្មីដោយ /resolve ឬសុំជំនួយ។"
INPUT_RESOLVE_STEP_TARGET_LABEL = "ជ្រើសតម្លៃ"
INPUT_RESOLVE_STEP_METHOD_LABEL = "ជ្រើសវិធី"
INPUT_RESOLVE_STEP_RAW_INPUT_LABEL = "បញ្ចូលអត្ថបទដើម"
INPUT_RESOLVE_STEP_CANDIDATES_LABEL = "ពិនិត្យជម្រើស"
INPUT_RESOLVE_STEP_DECISION_LABEL = "បញ្ជាក់ជម្រើស"
SUPPORT_ESCALATION_MESSAGE = "បានផ្ញើសំណើជំនួយទៅក្រុមប្រតិបត្តិការ។\nមិនបើកបង្អួចជជែកថ្មីទេ។\nអ្នកអាចបន្តក្នុងបង្អួចនេះ ឬប្រើ /help, /menu, /start។"
SUPPORT_HANDOFF_MESSAGE_RECORDED = "បានរក្សាទុកសារបន្ថែមក្នុងប្រវត្តិជំនួយ។\nអ្នកអាចបន្តទទួលការណែនាំក្នុងបង្អួចជជែកនេះ។\nប្រើ /start ប្រសិនបើចង់ចាប់ផ្តើមឡើងវិញ។"
SUPPORT_HANDOFF_CLOSED_MESSAGE = "ការជំនួយនេះបានបិទហើយ។\nបើត្រូវការ អ្នកអាចស្នើសុំម្ដងទៀតក្នុងបង្អួចជជែកនេះ។"
CANONICAL_WRITE_FAILURE_MESSAGE = "មានបញ្ហាក្នុងការរក្សាទុកព័ត៌មានបញ្ចូល។\nសូមព្យាយាមម្តងទៀត ឬប្រើ /support។"
GUIDED_FLOW_FERTILIZER = "បញ្ចូលជី"
GUIDED_FLOW_YIELD = "បញ្ចូលទិន្នផល"
GUIDED_FLOW_EVIDENCE = "ដាក់ស្នើភស្តុតាង"
GUIDED_FLOW_MYFIELDS = "ព័ត៌មានដីរបស់អ្នក"
GUIDED_FLOW_INPUT_RESOLVE = "បកស្រាយការបញ្ចូល"
GUIDED_FLOW_ONBOARDING = "ការចុះឈ្មោះ"
GUIDED_FLOW_SUPPORT = "ជំនួយ"
GUIDED_REVIEW_STAGE_LABEL = "ជំហានពិនិត្យ"
GUIDED_VALIDATING_STAGE_LABEL = "កំពុងពិនិត្យ"
GUIDED_SUMMARY_STAGE_LABEL = "ជំហានសង្ខេប"
GUIDED_RECEIVED_STAGE_LABEL = "បានទទួល"
GUIDED_OPERATOR_REPLY_STAGE_LABEL = "ចម្លើយពីប្រតិបត្តិករ"
GUIDED_CLOSED_STAGE_LABEL = "បានបិទ"
GUIDED_INPUT_MODE_BUTTON_ONLY = "ជ្រើសដោយប៊ូតុង"
GUIDED_INPUT_MODE_TEXT_ALLOWED = "បញ្ចូលអក្សរ"
GUIDED_INPUT_MODE_LOCATION_ATTACHMENT = "ផ្ញើទីតាំង"
GUIDED_INPUT_MODE_DOCUMENT_UPLOAD = "អាប់ឡូត document"
GUIDED_INPUT_MODE_STATUS_WAIT = "រង់ចាំការណែនាំ"
GUIDED_DRAFT_SUMMARY_PREFIX = "ទិន្នន័យបច្ចុប្បន្ន"
GUIDED_NEXT_ACTIONS_LABEL = "អ្វីដែលអាចធ្វើបន្ទាប់"
GUIDED_TEXT_VALUE_ONLY_HINT = "នៅជំហាននេះ សូមបញ្ចូលតែតម្លៃដែលត្រូវការ។"
GUIDED_TEXT_EXAMPLES_LABEL = "ឧទាហរណ៍"
GUIDED_TEXT_NOT_SUPPORTED_LABEL = "មិនគាំទ្រនៅជំហាននេះ"
ONBOARDING_NAME_EXAMPLES = ("សុខា", "ដារ៉ា គីម")
ONBOARDING_PHONE_EXAMPLES = ("+855 12 345 678", "+880 17 1234 5678")
ONBOARDING_NAME_UNSUPPORTED_INPUT_HINT = "ប្រយោគដែលបន្ថែមការណែនាំខ្លួនជាមួយឈ្មោះ ដូចជា \"ខ្ញុំឈ្មោះសុខា មកពីភ្នំពេញ\""
ONBOARDING_PHONE_UNSUPPORTED_INPUT_HINT = "ប្រយោគដែលមានលេខកូដប្រទេសមិនគាំទ្រ ឬពន្យល់បន្ថែម ដូចជា \"លេខខ្ញុំ 010-1234-5678\""
ONBOARDING_DRAFT_LANGUAGE_LABEL = "ភាសា"
ONBOARDING_DRAFT_NAME_LABEL = "ឈ្មោះ"
ONBOARDING_DRAFT_PHONE_LABEL = "លេខទូរស័ព្ទ"
MYFIELDS_DRAFT_BINDING_COUNT_LABEL = "ចំនួនដីបានចុះ"
MYFIELDS_DRAFT_METHOD_LABEL = "វិធីសាស្ត្រ"
MYFIELDS_DRAFT_CANDIDATE_COUNT_LABEL = "ចំនួនបេក្ខជន"
MYFIELDS_DRAFT_FIELD_LABEL = "ដី"
MYFIELDS_DRAFT_REQUESTED_CODE_LABEL = "លេខកូដដែលបានបញ្ចូល"
MYFIELDS_CODE_EXAMPLES = ("FIELD-001", "FIELD-105")
MYFIELDS_CODE_UNSUPPORTED_INPUT_HINT = "សំណើបែបពិពណ៌នា ដូចជា \"រកស្រែក្បែរខ្ញុំ\""
INPUT_RESOLVE_DRAFT_TARGET_LABEL = "គោលដៅ"
INPUT_RESOLVE_DRAFT_METHOD_LABEL = "វិធីបញ្ចូល"
INPUT_RESOLVE_DRAFT_RAW_INPUT_LABEL = "អត្ថបទដើម"
INPUT_RESOLVE_DRAFT_CANDIDATE_COUNT_LABEL = "ចំនួនបេក្ខជន"
EVIDENCE_LOCATION_ACCEPTED_LABEL = "បានទទួលទីតាំង"
EVIDENCE_FILE_NAME_LABEL = "ឈ្មោះឯកសារ"

HELP_MESSAGE = (
    "ពាក្យបញ្ជាដែលអាចប្រើបាន៖\n"
    "- /start : ចាប់ផ្តើមពីដើម\n"
    "- /help : បង្ហាញជំនួយ\n"
    "- /menu : បើកម៉ឺនុយមេ\n"
    "- /myfields : មើលព័ត៌មានរបស់អ្នក\n"
    "- /fertilizer : ចាប់ផ្តើមបញ្ចូលជី\n"
    "- /yield : ចាប់ផ្តើមបញ្ចូលទិន្នផល\n"
    "- /evidence : ចាប់ផ្តើមដាក់ស្នើភស្តុតាង\n"
    "- /resolve : ជួយបញ្ជាក់តម្លៃ\n"
    "- /support : បើកជំនួយ\n"
    "- /language : ប្តូរភាសា\n"
    "- ថយក្រោយ : ទៅជំហានមុន\n"
    "- ចាប់ផ្តើមឡើងវិញ : ចាប់ផ្តើមពីដើម"
)

FALLBACK_MESSAGES = {
    "default": "មិនអាចដំណើរការការបញ្ចូលនេះបានទេ។\nជ្រើសមុខងារខាងក្រោម។",
    "weather": "មិនអាចដំណើរការការបញ្ចូលនេះបានទេ។\nជ្រើសទីក្រុងខាងក្រោម ឬចុចថយក្រោយ។",
    "cancelled": "លំហូរនេះបានបិទហើយ។\nសូមជ្រើសមុខងារខាងក្រោមដើម្បីបន្ត។",
    "fertilizer_input": "មិនអាចដំណើរការការបញ្ចូលជីនេះបានទេ។\nជ្រើសធាតុជីខាងក្រោម ដើម្បីបន្ត ឬកែប្រែ។",
    "fertilizer_confirm": "អ្នកនៅជំហានបញ្ជាក់ជី។\nសូមជ្រើសបញ្ជាក់ ឬកែប្រែខាងក្រោម។",
    "yield_input": "មិនអាចដំណើរការការបញ្ចូលទិន្នផលនេះបានទេ។\nជ្រើសជំហានខាងក្រោម ដើម្បីបន្ត។",
    "yield_confirm": "អ្នកនៅជំហានបញ្ជាក់ទិន្នផល។\nសូមជ្រើសបញ្ជាក់ ឬថយក្រោយខាងក្រោម។",
    "myfields_input": "មិនអាចដំណើរការការចុះបញ្ជីស្រែនេះបានទេ។\nសូមជ្រើសវិធីដែលត្រូវនឹងជំហានបច្ចុប្បន្ន។",
    "myfields_confirm": "អ្នកនៅជំហានបញ្ជាក់ស្រែ។\nសូមជ្រើសបញ្ជាក់ ឬវិធីផ្សេងខាងក្រោម។",
    "evidence_input": "មិនអាចដំណើរការជំហានដាក់ស្នើភស្តុតាងបានទេ។\nសូមផ្ញើទីតាំង ឬ document upload ឲ្យត្រូវនឹងជំហាននេះម្ដងទៀត។",
    "evidence_status": "ឯកសារភស្តុតាងត្រូវបានទទួលរួចហើយ។\nសូមរង់ចាំការណែនាំបន្ទាប់ ឬជ្រើសសកម្មភាពដែលពាក់ព័ន្ធ។",
    "input_resolve_input": "មិនអាចដំណើរការជំហានបកស្រាយតម្លៃបានទេ។\nសូមជ្រើសប៊ូតុងដែលត្រូវ ឬបញ្ចូលតម្លៃខ្លីម្ដងទៀត។",
    "input_resolve_confirm": "នេះជាជំហានពិនិត្យជម្រើស។\nសូមជ្រើសជម្រើស សាកម្តងទៀត ត្រឡប់មកក្រោយ ឬឲ្យមនុស្សពិនិត្យ។",
    "onboarding_input": "មិនអាចដំណើរការការបញ្ចូលចុះឈ្មោះបានទេ។\nសូមបញ្ចូលឈ្មោះ ឬលេខទូរស័ព្ទសម្រាប់ជំហាននេះម្តងទៀត។",
    "onboarding_confirm": "នេះគឺជាជំហានពិនិត្យការចុះឈ្មោះ។\nសូមចុច [បញ្ជាក់] ឬជ្រើសធាតុដែលចង់កែ។",
    "onboarding_pending": "សំណើចុះឈ្មោះត្រូវបានទទួលហើយ។\nសូមរង់ចាំការណែនាំអំពីការអនុម័ត ឬជ្រើសសកម្មភាពដែលពាក់ព័ន្ធ។",
}
RECOVERY_GUIDANCE_SOFT = "នេះជាអ្វីដែលត្រូវបញ្ចូលនៅជំហាននេះ។"
RECOVERY_GUIDANCE_GUIDED = "សូមធ្វើតាមជំហានខាងក្រោម ដើម្បីបន្ត។"
RECOVERY_GUIDANCE_ESCALATION_READY = "បើបញ្ហានេះកើតឡើងម្តងទៀត អ្នកអាចជ្រើសធាតុខាងក្រោមបាន។"
RECOVERY_CURRENT_STEP_LABEL = "ជំហានបច្ចុប្បន្ន"
RECOVERY_QUICK_ACTIONS_LABEL = "ជម្រើសរហ័ស"
RECOVERY_BUTTON_ONLY_HINT = "ជំហាននេះត្រូវជ្រើសតាមប៊ូតុងខាងក្រោមប៉ុណ្ណោះ។"
RECOVERY_TEXT_ALLOWED_HINT = "ជំហាននេះអាចបញ្ចូលជាអក្សរបាន។"
RECOVERY_LOCATION_ATTACHMENT_HINT = "ជំហាននេះត្រូវភ្ជាប់ទីតាំងបច្ចុប្បន្ន។"
RECOVERY_DOCUMENT_UPLOAD_HINT = "ជំហាននេះត្រូវការអាប់ឡូត document។"
RECOVERY_STATUS_WAIT_HINT = "ជំហាននេះកំពុងរង់ចាំការណែនាំបន្ទាប់។"
RECOVERY_MULTI_SLOT_CANDIDATE_HINT = "រកឃើញតម្លៃជាច្រើនក្នុងសារតែមួយ។ ពិនិត្យសេចក្តីព្រាងខាងក្រោមមុនបន្ត។"
RECOVERY_MULTI_SLOT_APPLIED_MESSAGE = "តម្លៃជាច្រើនត្រូវបានដាក់ក្នុងសេចក្តីព្រាងបច្ចុប្បន្ន។ ពិនិត្យខាងក្រោម។"
CHEAP_GATE_SUPPORT_ESCALATE_MESSAGE = (
    "នេះជាសំណើសុំជំនួយ។\n"
    "សំណើត្រូវបានរក្សាទុកជាការបញ្ជូនទៅជំនួយ ហើយការណែនាំបន្តនឹងនៅក្នុងបង្អួចនេះ។\n"
    "ជ្រើសធាតុដែលពាក់ព័ន្ធខាងក្រោម ឬប្រើ /help។"
)
CHEAP_GATE_ADMIN_FOLLOWUP_MESSAGE = (
    "សំណើនេះត្រូវការការតាមដានបន្តពីប្រតិបត្តិការ។\n"
    "សំណើត្រូវបានរក្សាទុកសម្រាប់ការត្រួតពិនិត្យ ហើយការណែនាំបន្តនឹងនៅក្នុងបង្អួចនេះ។\n"
    "ជ្រើសធាតុដែលពាក់ព័ន្ធខាងក្រោមដើម្បីបន្ត។"
)
CHEAP_GATE_MANUAL_RESOLUTION_MESSAGE = (
    "ការបញ្ចូលនេះត្រូវបានពិនិត្យជាច្រើនដង ប៉ុន្តែមិនអាចដោះស្រាយដោយស្វ័យប្រវត្តិបានទេ។\n"
    "វាត្រូវបានរក្សាទុកសម្រាប់ការពិនិត្យប្រតិបត្តិការដោយដៃ។\n"
    "ជ្រើសធាតុដែលពាក់ព័ន្ធខាងក្រោម ឬចាប់ផ្តើមម្ដងទៀតដោយ /menu ឬ /start។"
)

PROFILE_CONFIRMED_MESSAGE = "បានបញ្ជាក់ការបញ្ចូល។\nបានរក្សាទុកក្នុងសម័យនេះ។"
PROFILE_EDIT_MESSAGE = "សូមជ្រើសព័ត៌មានដែលចង់កែ។"
PROFILE_EDIT_SELECTION_CONFIRMATION_MESSAGE = "ជ្រើសធាតុប្រវត្តិម្តងទៀត។\nចុច [{edit_button}] ដើម្បីបើកអេក្រង់កែប្រែ។"
PROFILE_NOT_FOUND_MESSAGE = "មិនទាន់មានប្រវត្តិដែលបានរក្សាទុកទេ។\nសូមបំពេញប្រវត្តិជាមុនសិន។"
PROFILE_NAME_PROMPT = "សូមបញ្ចូលឈ្មោះ។"
PROFILE_RESIDENCE_PROMPT = "សូមបញ្ចូលទីលំនៅ។"
PROFILE_CITY_PROMPT = "សូមបញ្ចូលរាជធានី/ខេត្ត។\nឧទាហរណ៍៖ ភ្នំពេញ"
PROFILE_DISTRICT_PROMPT = "សូមបញ្ចូលខណ្ឌ/ស្រុក/ក្រុង។\nឧទាហរណ៍៖ ខណ្ឌ៧មករា, សែនសុខ"
PROFILE_BIRTH_YEAR_PROMPT = "សូមជ្រើសឆ្នាំកំណើតពីប៊ូតុង។"
PROFILE_BIRTH_MONTH_PROMPT = "សូមជ្រើសខែកំណើតពីប៊ូតុង។"
PROFILE_BIRTH_DAY_PROMPT = "សូមជ្រើសថ្ងៃកំណើតពីប៊ូតុង។"
PROFILE_NAME_FALLBACK = "សូមបញ្ចូលឈ្មោះម្ដងទៀត។"
PROFILE_RESIDENCE_FALLBACK = "សូមបញ្ចូលទីលំនៅម្ដងទៀត។"
PROFILE_CITY_FALLBACK = "សូមបញ្ចូលរាជធានី/ខេត្តឲ្យត្រឹមត្រូវម្ដងទៀត។"
PROFILE_DISTRICT_FALLBACK = "សូមបញ្ចូលខណ្ឌ/ស្រុក/ក្រុងឲ្យត្រឹមត្រូវម្ដងទៀត។"
PROFILE_BIRTH_YEAR_FALLBACK = "សូមជ្រើសឆ្នាំកំណើតពីប៊ូតុង។"
PROFILE_BIRTH_MONTH_FALLBACK = "សូមជ្រើសខែកំណើតពីប៊ូតុង។"
PROFILE_BIRTH_DAY_FALLBACK = "សូមជ្រើសថ្ងៃកំណើតពីប៊ូតុង។"
PROFILE_CONFIRM_FALLBACK = "សូមជ្រើសរើស [បញ្ជាក់] ឬ [កែប្រែ]។"
PROFILE_EDIT_SELECT_FALLBACK = "សូមជ្រើសព័ត៌មានដែលត្រូវកែពីប៊ូតុង។"
PROFILE_REPAIR_NAME_MESSAGE = "យើងនឹងបញ្ចូលឈ្មោះម្តងទៀត។"
PROFILE_REPAIR_RESIDENCE_MESSAGE = "យើងនឹងបញ្ចូលទីលំនៅម្តងទៀត។"
PROFILE_REPAIR_CITY_MESSAGE = "យើងនឹងបញ្ចូលរាជធានី/ខេត្តម្តងទៀត។"
PROFILE_REPAIR_DISTRICT_MESSAGE = "យើងនឹងបញ្ចូលខណ្ឌ/ស្រុក/ក្រុងម្តងទៀត។"
PROFILE_REPAIR_BIRTH_MESSAGE = "យើងនឹងបញ្ចូលថ្ងៃខែឆ្នាំកំណើតម្តងទៀត។\nសូមជ្រើសឆ្នាំកំណើត។"
PROFILE_DIRECT_UPDATE_MESSAGE = "ការផ្លាស់ប្តូរត្រូវបានរក្សាទុក។ ពិនិត្យ ហើយចុច [បញ្ជាក់]។"
LLM_REPAIR_CANDIDATE_HINT = "បានរកឃើញសំណើប្តូរទៅ \"{candidate_value}\"។\nបើត្រឹមត្រូវ សូមចុច ចាប់ផ្តើមកែ។"
LLM_REPAIR_UNSUPPORTED_MESSAGE = "មិនអាចកំណត់ធាតុដែលត្រូវកែបានទេ។\nសូមជ្រើសដោយផ្ទាល់ពីធាតុខាងក្រោម។"
LLM_REPAIR_LOW_CONFIDENCE_MESSAGE = "មិនអាចបញ្ជាក់ធាតុដែលត្រូវកែបានទេ។\nសូមជ្រើសដោយផ្ទាល់ពីធាតុខាងក្រោម។"
LLM_REPAIR_HUMAN_REVIEW_MESSAGE = (
    "សំណើនេះពិបាកកាត់ឲ្យសល់តែធាតុកែប្រែមួយដោយស្វ័យប្រវត្តិ។\n"
    "វាអាចត្រូវការមនុស្សពិនិត្យ ដូច្នេះសូមជ្រើសដោយផ្ទាល់ពីធាតុខាងក្រោម។"
)
LLM_REPAIR_RUNTIME_FAILURE_MESSAGE = (
    "ការបកស្រាយដោយស្វ័យប្រវត្តិមិនទាន់ស្ថិតស្ថេរល្អទេ។\n"
    "សូមបន្តដោយជ្រើសដោយផ្ទាល់ពីធាតុខាងក្រោម។"
)

FERTILIZER_KIND_LABELS = {
    "compound": "ជីចម្រុះ",
    "urea": "យូរ៉េ",
    "compost": "ជីកំប៉ុស",
    "liquid": "ជីរាវ",
}
FERTILIZER_USED_LABEL_YES = "បានប្រើ"
FERTILIZER_USED_LABEL_NO = "មិនបានប្រើ"
FERTILIZER_USED_PROMPT = "តើបានប្រើជីឬនៅ?"
FERTILIZER_KIND_PROMPT = "សូមជ្រើសប្រភេទជី។"
FERTILIZER_PRODUCT_PROMPT = "សូមបញ្ចូលឈ្មោះផលិតផល។"
FERTILIZER_AMOUNT_PROMPT = "សូមបញ្ចូលបរិមាណដែលបានប្រើ។"
FERTILIZER_DATE_PROMPT = "សូមបញ្ចូលថ្ងៃប្រើជី។"
FERTILIZER_PRODUCT_EXAMPLES = ("Compound 21-17-17", "Liquid-A")
FERTILIZER_AMOUNT_EXAMPLES = ("20kg", "1 bag")
FERTILIZER_DATE_EXAMPLES = ("2026-04-21", "today")
FERTILIZER_PRODUCT_UNSUPPORTED_INPUT_HINT = "ប្រយោគដែលបញ្ចូលព័ត៌មានច្រើនក្នុងពេលតែមួយ ដូចជា \"បានប្រើជី 20kg ម្សិលមិញ\""
FERTILIZER_AMOUNT_UNSUPPORTED_INPUT_HINT = "ពាក្យមិនច្បាស់ដែលគ្មានលេខ ឬ ឯកតា ដូចជា \"ប្រើបន្តិច\""
FERTILIZER_DATE_UNSUPPORTED_INPUT_HINT = "ពាក្យបង្ហាញតែជួរពេលវេលា ដូចជា \"ប្រហែលសប្តាហ៍មុន\""
FERTILIZER_CONFIRM_PROMPT = "សូមពិនិត្យការបញ្ចូលជី។"
FERTILIZER_USED_FALLBACK = "សូមជ្រើសរើស បាទ/ចាស ឬ ទេ។"
FERTILIZER_KIND_FALLBACK = "សូមជ្រើសប្រភេទជីពីប៊ូតុង។"
FERTILIZER_PRODUCT_FALLBACK = "សូមបញ្ចូលឈ្មោះផលិតផលម្ដងទៀត។"
FERTILIZER_AMOUNT_FALLBACK = "សូមបញ្ចូលបរិមាណម្ដងទៀត។"
FERTILIZER_DATE_FALLBACK = "សូមបញ្ចូលថ្ងៃប្រើជីម្ដងទៀត។"
FERTILIZER_CONFIRM_FALLBACK = "សូមចុច [បញ្ជាក់] ឬថយក្រោយ ដើម្បីកែប្រែ។"
FERTILIZER_CONFIRMED_MESSAGE = "បានរក្សាទុកសេចក្តីព្រាងជីក្នុងសម័យនេះ។"
FERTILIZER_EDIT_MESSAGE = "សូមជ្រើសធាតុជីដែលចង់កែ។"
FERTILIZER_EDIT_SELECTION_CONFIRMATION_MESSAGE = "ជ្រើសធាតុជីម្តងទៀត។\nចុច [{edit_button}] ដើម្បីបើកអេក្រង់កែប្រែ។"
FERTILIZER_NOT_FOUND_MESSAGE = "មិនទាន់មានទិន្នន័យជីដែលបានរក្សាទុកទេ។\nសូមបំពេញការបញ្ចូលជាមុនសិន។"
FERTILIZER_REPAIR_USED_MESSAGE = "យើងនឹងបញ្ចូលស្ថានភាពប្រើជីម្តងទៀត។"
FERTILIZER_REPAIR_KIND_MESSAGE = "យើងនឹងបញ្ចូលប្រភេទជីម្តងទៀត។"
FERTILIZER_REPAIR_PRODUCT_MESSAGE = "សូមបញ្ចូលឈ្មោះផលិតផលថ្មី។"
FERTILIZER_REPAIR_AMOUNT_MESSAGE = "សូមបញ្ចូលបរិមាណថ្មី។"
FERTILIZER_REPAIR_DATE_MESSAGE = "សូមបញ្ចូលថ្ងៃប្រើថ្មី។"
FERTILIZER_DIRECT_UPDATE_MESSAGE = "ការផ្លាស់ប្តូរត្រូវបានរក្សាទុក។ ពិនិត្យ ហើយចុច [បញ្ជាក់]។"
YIELD_READY_PROMPT = "តើបានប្រមូលផលរួចហើយឬនៅ?"
YIELD_FIELD_PROMPT = "សូមបញ្ចូលឈ្មោះស្រែដែលបានប្រមូលផល។"
YIELD_AMOUNT_PROMPT = "សូមបញ្ចូលបរិមាណប្រមូលផល។"
YIELD_DATE_PROMPT = "សូមបញ្ចូលថ្ងៃប្រមូលផល។"
YIELD_FIELD_EXAMPLES = ("Plot 1", "FIELD-001")
YIELD_AMOUNT_EXAMPLES = ("320kg", "1.2t")
YIELD_DATE_EXAMPLES = ("2026-04-21", "today")
YIELD_FIELD_UNSUPPORTED_INPUT_HINT = "ការពិពណ៌នាមិនច្បាស់ ដូចជា \"ស្រែដែលខ្ញុំទើបប្រមូល\""
YIELD_AMOUNT_UNSUPPORTED_INPUT_HINT = "ពាក្យដែលគ្មានលេខ ដូចជា \"ប្រមូលបានច្រើន\""
YIELD_DATE_UNSUPPORTED_INPUT_HINT = "ពាក្យទូលំទូលាយ ដូចជា \"ប៉ុន្មានថ្ងៃមុន\""
YIELD_CONFIRM_PROMPT = "សូមពិនិត្យការបញ្ចូលទិន្នផល។"
YIELD_READY_FALLBACK = "សូមជ្រើសរើស បាទ/ចាស ឬ ទេ។"
YIELD_FIELD_FALLBACK = "សូមបញ្ចូលស្រែដែលបានប្រមូលផលម្ដងទៀត។"
YIELD_AMOUNT_FALLBACK = "សូមបញ្ចូលបរិមាណប្រមូលផលម្ដងទៀត។"
YIELD_DATE_FALLBACK = "សូមបញ្ចូលថ្ងៃប្រមូលផលម្ដងទៀត។"
YIELD_CONFIRM_FALLBACK = "សូមចុច [បញ្ជាក់] ឬថយក្រោយ ដើម្បីកែប្រែ។"
YIELD_EDIT_MESSAGE = "សូមជ្រើសធាតុប្រមូលផលដែលចង់កែ។"
YIELD_EDIT_SELECT_FALLBACK = "សូមជ្រើសធាតុដែលត្រូវកែ ឬ ចុច [ចាប់ផ្តើមកែ] ឬថយក្រោយ។"
YIELD_CONFIRMED_MESSAGE = "បានរក្សាទុកសេចក្តីព្រាងទិន្នផលក្នុងសម័យនេះ។"
YIELD_REPAIR_FIELD_MESSAGE = "សូមបញ្ចូលឈ្មោះស្រែថ្មី។"
YIELD_REPAIR_AMOUNT_MESSAGE = "សូមបញ្ចូលបរិមាណប្រមូលផលថ្មី។"
YIELD_REPAIR_DATE_MESSAGE = "សូមបញ្ចូលថ្ងៃប្រមូលផលថ្មី។"
INPUT_RESOLVE_TARGET_PROMPT = "សូមជ្រើសតម្លៃដែលចង់បញ្ជាក់។"
INPUT_RESOLVE_METHOD_PROMPT = "សូមជ្រើសវិធីបញ្ចូល។\nឥឡូវនេះអាចប្រើបានតែ [បញ្ចូលអក្សរ] ប៉ុណ្ណោះ។"
INPUT_RESOLVE_RAW_INPUT_PROMPT = "សូមបញ្ចូលតម្លៃដែលចង់កំណត់តាមដើម។"
INPUT_RESOLVE_RAW_INPUT_EXAMPLES = ("FIELD-001", "Plot 1")
INPUT_RESOLVE_RAW_INPUT_UNSUPPORTED_INPUT_HINT = "ប្រយោគដែលពិពណ៌នាប៉ុណ្ណោះ ដូចជា \"ជ្រើសអ្វីដែលជិតបំផុតឲ្យខ្ញុំ\""
INPUT_RESOLVE_CANDIDATES_PROMPT = "សូមពិនិត្យជម្រើស ហើយជ្រើស 1-3 ឬសាកល្បងម្តងទៀត។"
INPUT_RESOLVE_DECISION_PROMPT = "សូមជ្រើស បញ្ជាក់រួច សាកល្បងម្តងទៀត ឬឲ្យមនុស្សពិនិត្យ។"


def format_profile_confirmation(
    *,
    name: str,
    birth_date: str,
    city: str,
    district: str,
    residence: str,
) -> str:
    return (
        "សូមពិនិត្យទិន្នន័យ។\n"
        f"- ឈ្មោះ: {name}\n"
        f"- ទីលំនៅ: {residence}\n"
        f"- រាជធានី/ខេត្ត: {city}\n"
        f"- ខណ្ឌ/ស្រុក/ក្រុង: {district}\n"
        f"- ថ្ងៃខែឆ្នាំកំណើត: {birth_date}\n"
        "\nបើត្រឹមត្រូវ សូមចុច [បញ្ជាក់]។ បើមិនត្រឹមត្រូវ សូមចុច [កែប្រែ]។"
    )


def format_repair_confirmation(*, field_label: str, edit_button: str) -> str:
    return (
        f"ចង់កែធាតុ {field_label} មែនទេ?\n"
        f"ចុច [{edit_button}] ដើម្បីបញ្ចូលម្តងទៀត។"
    )


def format_change_preview(*, field_label: str, before_value: str, after_value: str) -> str:
    return (
        f"សូមពិនិត្យការផ្លាស់ប្តូរ {field_label}។\n"
        f"- មុន: {before_value}\n"
        f"- ថ្មី: {after_value}"
    )


def format_profile_summary(
    *,
    name: str,
    birth_date: str,
    city: str,
    district: str,
    residence: str,
) -> str:
    return (
        "នេះជាប្រវត្តិដែលបានរក្សាទុក។\n"
        f"- ឈ្មោះ: {name}\n"
        f"- ទីលំនៅ: {residence}\n"
        f"- រាជធានី/ខេត្ត: {city}\n"
        f"- ខណ្ឌ/ស្រុក/ក្រុង: {district}\n"
        f"- ថ្ងៃខែឆ្នាំកំណើត: {birth_date}"
    )


def format_fertilizer_confirmation(
    *,
    used: bool | None,
    kind_label: str,
    product_name: str,
    amount_text: str,
    applied_date: str,
) -> str:
    if used is False:
        return (
            "សូមពិនិត្យការបញ្ចូលជី។\n"
            "- បានប្រើជី: ទេ\n\n"
            "បើត្រឹមត្រូវ សូមចុច [បញ្ជាក់]។"
        )
    return (
        "សូមពិនិត្យការបញ្ចូលជី។\n"
        f"- បានប្រើជី: {'បាទ/ចាស' if used else '-'}\n"
        f"- ប្រភេទ: {kind_label}\n"
        f"- ផលិតផល: {product_name}\n"
        f"- បរិមាណ: {amount_text}\n"
        f"- កាលបរិច្ឆេទ: {applied_date}\n\n"
        "បើត្រឹមត្រូវ សូមចុច [បញ្ជាក់]។"
    )


def format_fertilizer_summary(
    *,
    used: bool | None,
    kind_label: str,
    product_name: str,
    amount_text: str,
    applied_date: str,
) -> str:
    if used is False:
        return (
            "នេះជាការបញ្ចូលជីដែលបានរក្សាទុក។\n"
            "- បានប្រើជី: ទេ"
        )
    return (
        "នេះជាការបញ្ចូលជីដែលបានរក្សាទុក។\n"
        f"- បានប្រើជី: {'បាទ/ចាស' if used else '-'}\n"
        f"- ប្រភេទ: {kind_label}\n"
        f"- ផលិតផល: {product_name}\n"
        f"- បរិមាណ: {amount_text}\n"
        f"- កាលបរិច្ឆេទ: {applied_date}"
    )


def format_yield_confirmation(
    *,
    ready: bool | None,
    field_name: str,
    amount_text: str,
    harvest_date: str,
) -> str:
    return (
        "សូមពិនិត្យការបញ្ចូលទិន្នផល។\n"
        f"- រួចរាល់ឬនៅ: {'រួចរាល់' if ready else 'មិនទាន់រួចរាល់' if ready is False else '-'}\n"
        f"- ស្រែ: {field_name}\n"
        f"- បរិមាណ: {amount_text}\n"
        f"- កាលបរិច្ឆេទប្រមូលផល: {harvest_date}\n\n"
        "បើត្រឹមត្រូវ សូមចុច [បញ្ជាក់]។"
    )


def format_support_admin_reply(*, admin_message: str) -> str:
    return (
        "ចម្លើយពីក្រុមប្រតិបត្តិការ។\n"
        f"{admin_message}\n\n"
        "បើត្រូវការ សូមបន្តសរសេរក្នុងបង្អួចនេះ។"
    )


def format_input_resolve_candidate_list(*, target_label: str, raw_input: str, candidates: tuple[str, ...]) -> str:
    candidate_lines = "\n".join(f"{index}. {candidate}" for index, candidate in enumerate(candidates, start=1))
    return (
        f"បានរកឃើញជម្រើសសម្រាប់ {target_label}។\n"
        f"- តម្លៃដើម: {raw_input}\n\n"
        f"{candidate_lines}\n\n"
        "សូមជ្រើសជម្រើសដែលត្រូវ ឬសាកម្តងទៀត។"
    )


def format_input_resolve_selected_candidate(*, target_label: str, candidate_label: str) -> str:
    return (
        f"សូមពិនិត្យជម្រើស {target_label}។\n"
        f"- ជម្រើសដែលបានជ្រើស: {candidate_label}\n\n"
        "សូមជ្រើសយកជម្រើសនេះ សាកម្តងទៀត ពេលក្រោយ ឬឲ្យមនុស្សពិនិត្យ។"
    )


def format_input_resolve_resolved(*, target_label: str, candidate_label: str) -> str:
    return (
        "តម្លៃដែលបានបកស្រាយត្រូវបានរក្សាទុកហើយ។\n"
        f"- គោលដៅ: {target_label}\n"
        f"- តម្លៃ: {candidate_label}"
    )


def format_input_resolve_reminder(*, command_text: str) -> str:
    return (
        "ដល់ពេលបន្តលំហូរបកស្រាយតម្លៃដែលបានរក្សាទុកហើយ។\n"
        f"សូមបញ្ចូល {command_text} ដើម្បីបើកឡើងវិញ។"
    )


def format_input_resolve_reminder_created(*, command_text: str) -> str:
    return (
        "បានរក្សាទុកសម្រាប់បន្តនៅពេលក្រោយ។\n"
        f"ពេលចង់បន្ត សូមបញ្ចូល {command_text}។"
    )


def format_input_resolve_resumed(*, step_label: str, prompt_text: str) -> str:
    return f"បានបើកលំហូរបកស្រាយតម្លៃដែលបានរក្សាទុកឡើងវិញ។\nជំហានបច្ចុប្បន្ន: {step_label}\n\n{prompt_text}"


def format_evidence_entry(*, field_label: str) -> str:
    return (
        f"{EVIDENCE_ENTRY_MESSAGE}\n"
        f"- ស្រែដែលបានភ្ជាប់បច្ចុប្បន្ន: {field_label}"
    )


def format_evidence_uploaded(*, file_name: str) -> str:
    return (
        "បានទទួលឯកសារភស្តុតាងហើយ។\n"
        f"- ឈ្មោះឯកសារ: {file_name}\n\n"
        "ជំហានបន្ទាប់នឹងបន្តក្នុងលំហូរនេះ នៅពេលការត្រួតពិនិត្យមូលដ្ឋានរួចរាល់។"
    )


def format_evidence_accepted(*, file_name: str) -> str:
    return (
        f"{EVIDENCE_ACCEPTED_MESSAGE}\n"
        f"- ឈ្មោះឯកសារ: {file_name}"
    )


def format_evidence_retry_required(*, file_name: str, reason_lines: tuple[str, ...]) -> str:
    reason_text = "\n".join(f"- {line}" for line in reason_lines)
    return (
        f"{EVIDENCE_RETRY_REQUIRED_MESSAGE}\n"
        f"- ឈ្មោះឯកសារ: {file_name}\n"
        f"{reason_text}\n\n"
        "សូមអាប់ឡូត document ម្ដងទៀតនៅជំហានដដែលនេះ។"
    )


def format_evidence_manual_review(*, file_name: str, reason_lines: tuple[str, ...]) -> str:
    reason_text = "\n".join(f"- {line}" for line in reason_lines)
    reason_block = f"{reason_text}\n\n" if reason_text else ""
    return (
        f"{EVIDENCE_MANUAL_REVIEW_MESSAGE}\n"
        f"- ឈ្មោះឯកសារ: {file_name}\n"
        f"{reason_block}"
        "ការណែនាំបន្តនឹងបន្តនៅក្នុងប្រអប់ជជែកនេះ។"
    )


WEATHER_CODE_LABELS = {
    0: "មេឃស្រឡះ",
    1: "ភាគច្រើនស្រឡះ",
    2: "មានពពកខ្លះ",
    3: "មានពពកច្រើន",
    45: "អ័ព្ទ",
    48: "អ័ព្ទកក",
    51: "ភ្លៀងរលឹមស្រាល",
    53: "ភ្លៀងរលឹម",
    55: "ភ្លៀងរលឹមខ្លាំង",
    56: "ភ្លៀងកកស្រាល",
    57: "ភ្លៀងកកខ្លាំង",
    61: "ភ្លៀងស្រាល",
    63: "ភ្លៀង",
    65: "ភ្លៀងខ្លាំង",
    66: "ភ្លៀងកកស្រាល",
    67: "ភ្លៀងកកខ្លាំង",
    71: "ព្រិលស្រាល",
    73: "ព្រិល",
    75: "ព្រិលខ្លាំង",
    77: "គ្រាប់ព្រិល",
    80: "ភ្លៀងបក់ស្រាល",
    81: "ភ្លៀងបក់",
    82: "ភ្លៀងបក់ខ្លាំង",
    85: "ព្រិលបក់ស្រាល",
    86: "ព្រិលបក់ខ្លាំង",
    95: "ព្យុះផ្គរ",
    96: "ព្យុះផ្គរជាមួយគ្រាប់ទឹកកកស្រាល",
    99: "ព្យុះផ្គរជាមួយគ្រាប់ទឹកកកខ្លាំង",
}


def weather_code_label(code: int) -> str:
    return WEATHER_CODE_LABELS.get(code, f"ស្ថានភាពមិនស្គាល់ ({code})")


def format_today_date(current: datetime) -> str:
    weekdays = ["ច", "អ", "ព", "ព្រហ", "សុ", "សៅ", "អាទិ"]
    weekday = weekdays[current.weekday()]
    return (
        "ថ្ងៃនេះ។\n"
        f"- កាលបរិច្ឆេទ: {current.strftime('%Y-%m-%d')} ({weekday})\n"
        f"- ម៉ោង: {current.strftime('%H:%M:%S')} KST"
    )


def format_weather(
    *,
    city: str,
    observed_at: datetime,
    weather_label: str,
    temperature: float,
    apparent_temperature: float,
    wind_speed: float,
) -> str:
    return (
        f"អាកាសធាតុបច្ចុប្បន្ននៅ {city}។\n"
        f"- ពេលសង្កេត: {observed_at.strftime('%Y-%m-%d %H:%M')} KST\n"
        f"- ស្ថានភាព: {weather_label}\n"
        f"- សីតុណ្ហភាព: {temperature:.1f}°C\n"
        f"- មានអារម្មណ៍ដូចជា: {apparent_temperature:.1f}°C\n"
        f"- ល្បឿនខ្យល់: {wind_speed:.1f} km/h"
    )
