...
def test_admin_audit_repository_can_filter_by_occurred_date(tmp_path):
    runtime = bootstrap_sqlite_runtime(
        SqliteSettings(
            database_path=str(tmp_path / "runtime.sqlite3"),
            migrations_enabled=True,
    ...
    try:
        ...
        target = repository.record_event(
            action_code="admin.access.denied",
            actor_type_code="unknown",
            actor_id=None,
            result_code=RESULT_FAILURE,
        ...
        events = repository.list_events(occurred_from="2026-04-22", occurred_to="2026-04-22")
