...
def test_start_with_revoked_invitation_does_not_create_onboarding_session(monkeypatch, tmp_path):
    sent: list[str] = []
    ...
    try:
        ...
        asyncio.run(commands.start_command(_update(f"/start {revoked.invite_code}"), context))
