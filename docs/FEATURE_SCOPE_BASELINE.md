...
이 흐름은 제품 온보딩 계약으로 해석하지 않는다.

또한 이 샘플 흐름은 farmer-facing product surface에 직접 노출하지 않는다.

- 메인 메뉴에 프로필 진입 버튼을 두지 않는다.
- `/profile`을 제품 명령 surface로 지원하지 않는다.
- 전역 자연어 repair나 조회 routing에서 프로필 intent를 우선 해석하지 않는다.
- 프로필 상태가 아닌 곳에서 프로필 전용 fallback을 노출하지 않는다.

제품 온보딩은 상위 참고 기준의 이름, 전화번호, 선호 언어, 지원 국가 코드 기반 전화번호 정규화 규칙을 따라야 한다.

따라서 프로필 샘플의 거주지, 시/도, 구/군/시, 생년월일 입력은 실제 온보딩 명세와 병합하지 않고 샘플 범위로 격리한다.
...
비-SQLite 로컬 런타임에서 `/start`는 시작 게이트일 뿐 로컬 샘플 로그인 명령이 아니다. `/start <sample_id>` 형태의 임시 로그인 해석은 구현 대상에서 제외한다.
