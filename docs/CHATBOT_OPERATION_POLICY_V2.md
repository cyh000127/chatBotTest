...
- retry가 특정 필드 재입력 성격이면 `현재 값 1개 또는 현재 문맥 요약 + 새 값 입력`으로 줄인다.
- 이미 target, method, lookup path가 정해진 상태에서는 retry가 전체 draft를 다시 설명하지 않는다.
- 후보 없음 상태와 실제 재입력 상태는 분리한다. 후보 없음은 다음 행동 버튼을 먼저 보여주고, `다시 입력` 선택 후에만 repair prompt로 이동한다.
