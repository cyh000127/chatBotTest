...
- handoff는 새 채팅방 생성이 아니다.
- handoff는 실시간 상담 연결 보장이 아니다.
- handoff 이후에도 사용자는 기존 챗봇 대화창을 유지한다.
- 운영자 후속 답변은 봇이 기존 챗봇 대화창으로 전달한다.
- handoff 상태의 사용자 추가 메시지는 같은 이관 항목에 이어서 기록한다.
