...
- `AI_MODE=manual_review_fallback`이면 LLM 객체를 만들지 않고 runtime mode를 `rules_only_manual_review`로 고정한다.
...
현재 helper gate는 아래처럼 해석한다.

- `disabled`
  - rules-only, 일반 비활성화 상태
- `manual_review_fallback`
  - rules-only + manual review fallback 상태
- `repair_assist_only`
  - edit-intent 보조 호출만 허용
- `recovery_assist_only`
  - recovery classifier만 허용
...
- `src/PROJECT/channels/telegram/app.py`
  ...
  `llm_runtime_mode`, `manual_review_fallback_active` 같은 runtime flag도 함께 노출
...
7. Gemini 호출
8. 실패 시 `llm_failed` telemetry와 rules-only fallback 강제
9. JSON 결과를 validator와 state machine으로 다시 검증
