...
이 런타임은 자유 대화형 엔진이 아니라 guided structured interaction을 지향한다.

따라서 UX 개편도 아래 방향을 따른다.

- 현재 단계와 남은 입력을 짧게 보여주는 진행 상태 표면
- 버튼 전용 단계와 텍스트 입력 허용 단계의 명시적 구분
- 첫 진입 copy와 retry copy의 분리
- 실패 이유, 허용 입력 형식, 다음 행동이 함께 보이는 state-aware fallback
- resume, support, manual review 대기 상태에서의 안전한 다음 행동 안내

즉 UX 개선의 목적은 state machine을 숨기는 것이 아니라, 사용자가 현재 상태와 다음 행동을 더 쉽게 이해하도록 만드는 것이다.
